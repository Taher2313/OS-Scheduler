...
void SRTN(queue* Ready_Queue)
{
    ...
    {	
		...
		//the parent continue work in the algorithm
	     ...
	     int index_leastRemainingTime=0;
	     for(int k=0 k<Ready_Queue.length; k++)
	     {
		if(Ready_Queue[k].pcb->remaining_time < Ready_Queue[index_leastRemainingTime].pcb->remaining_time)
			{
			index_leastRemainingTime=k;
			}
		}
	     if((currentProcess != k)
		{
